...
function householderRelationChange(newValue){
    if(newValue==1){//户主
        $("#hzId").lookup("setTxt","");
        $("#hzId").lookup("setVal","");
        $("#familytree").combobox("setValue","");
        $("#familytree").combobox("disable");
    }
}
...
function updateQueryParam(){
	...
	var householderRelation = $.getUrlParam("q_householderRelation_EQ");
	if(gllx){
		...
		if(householderRelation){
			$("#queryForm").append("<input type='hidden' name='q_householderRelation_EQ' value='"+ householderRelation +"'>");
		}
	}
}
...
var baseInfo =  {
	...
	edit:function (){
		...
		setFormItemDisabled("#baseForm",false,exclude);
        var relation = $("#householderRelation").combobox("getValue");
        if(relation!=1){
            $("#familytree").combobox("enable");
        }else{
            $("#familytree").combobox("disable");
        }
	},
	...
	if(row){
		...
		if(id){
			var ajax = publicFormControl.ajax({
				...
				$("#baseForm").form("load",row);
				$("#houseId").lookup("setTxt",row.dyCode);
                $("#hzId").lookup("setTxt",row.hzName);
                $("#hzId").lookup("setVal",row.hzId);
				baseInfo.initAge();
				baseInfo.showImg();
				otherInfo.init();
				otherInfo.linkBtnStatus();
...
var houseHolderLookup;

function houseHolderLookup() {

	houseHolderLookup = $("#hzId").lookup({
		title: "选择户主",
		url: thisUiConfig.ctx + "/householder/selectHz",
		width: 700,
		height: 440,
		contentWindow: {
			params: {
			...
			actions: {
				confirm: function(data) {
					if (data) {
						$("#hzId").lookup("setVal",data.householderId);
						$("#hzId").lookup("setTxt",data.householderName);
                        $("#familytree").combobox("enable");
					}else{
                        $("#familytree").combobox("disable");
                    }
		...
		onBeforeOpen: function() {
			var orgId = getOrgid() ;
            var v = $("#householderRelation").combobox("getValue");
			if(orgId){

                if(v==1){
                    main.alert("温馨提示","房主无法选择户主名称！")
                    return false;
                }
			...
			$("#listGrid").datagrid({
				...
				columns : [ [ {
					...
					formatter : function (value,row,index){
						...
						return publicAttr.getDictText("HOUSEHOLDER",value);
					 }
				},{
					field:"callPhone",
					title:"联系电话",
