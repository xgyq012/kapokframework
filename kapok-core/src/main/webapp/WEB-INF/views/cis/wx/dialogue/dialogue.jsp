            ...
            <form id="dataForm" class="formCls" method="post">
                ...
                <div id="sub" style="height:500px;">
                    <div  id="subTabs" class="easyui-tabs" data-options="fit:true,border:false"> 
                        <div title="对话明细" style="overflow:hidden;">
                            <div id="buttonbar">
                               	内容搜索:<input id="recordContent" name="recordContent"type="text">
                               <a id="subSearch" href="javascript:void(0)" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-search'"  onclick="getWxDialogueLs();" >查询</a>
                           ...
                           <table id="dg_wxDialogueLs" class="easyui-datagrid" style="width:auto;height: 432px;"
                                    data-options="
                                       	idField : 'recordLId',
										rownumbers : true,
										singleSelect : false,
										autoRowHeight : false,
										toolbar : '#buttonbar',
										onClickRow : ondg_wxDialogueLsClickRow,
										fitcolumns:true, striped:true">
                                    <thead>
                                        <tr>
                                        	<th data-options="field:'recordLId',hidden:true">主键</th>
                                        	<th data-options="field:'recordId',hidden:true">头表主键id</th>
											<th data-options="field:'createrId',hidden:true">创建人ID</th>
                                            <th data-options="field:'recordType',width:100" >内容类型</th>
                                            <th data-options="field:'recordContent',width:100" >发送内容</th>
                                            <th data-options="field:'createTime',formatter: formatterDate">发送时间</th>
                                        </tr>
                                    </thead>
                                </table>
                        </div>
...
<script type="text/javascript">
	...
	$(function(){
	    ...
	    $("#listGrid").datagrid({
			...
			pagination : true,
			url : "${ctx}/dialogue/search",
				columns : [[
					{field : "recordId", title : "对话记录主表主键"},
					{field : "subscription", title : "公众号帐号"},
					{field : "subscribeNickname", title : "用户昵称"}
			]],
	    ...
	    $("#dataForm").form({
	        onLoadSuccess: function(data) {
	        	/* if (data.wxDialogueRecordL) {
	        		$("#recordId").attr("value",data.recordId);alert($("#recordId").val())
		            $("#dg_wxDialogueLs").datagrid("loadData", data.wxDialogueRecordL);
	        	} */ 
	        	$("#recordId").attr("value",data.recordId);
	            $("#dg_wxDialogueLs").datagrid("loadData", data);
	        	disableItem(disabledItem);
	        }
	    });
	    
	    $("#detailTab").click(function(){
	    	$("#dg_wxDialogueLs").datagrid("acceptChanges");
			$("#dg_wxDialogueLs").datagrid("unselectAll");
		});
	    
	});

	//获取对话明细记录
	function getWxDialogueLs(){
		$("#dg_wxDialogueLs").datagrid("load", getFormData("#dataForm"));
    ...
    function loadDictData() {
        ...
        if (row) {
        	...
        	$.ajax({
    			type: "GET",
    			url: "${ctx}/dialogue/getDialogueLByPage?recordId=" + row.recordId,
    			//url: "${ctx}/dialogue/get/",
    			data:data
    		}).done(function(res){
    			$.messager.progress("close");
    			if (res.resultCode == "0") {
    				$("#dataForm").form("load", res);
		        	$("#mainTabs").tabs("select", 1);
		        	$('#dg_wxDialogueLs').datagrid({
						url:'${ctx}/dialogue/getDialogueLByPage',
						border : false,
						pageSize : defaultPageSize,
						pageList : defaultPageList,
						pagination : true,
    ...
    function add(){
		...
		disableItem(disabledItem);
        $("#dataForm").form("clear");
        $('#dg_wxDialogueLs').datagrid('loadData',{total:0,rows:[]});
        $("#listGrid").datagrid("unselectAll");
        $("#mainTabs").tabs("enableTab", 1);
        $("#mainTabs").tabs("select", 1);
        $("#dictTypeCode").focus();
	...
	function disableItem(disable) {
		...
		$("#dg_wxDialogueLs").datagrid({
			onClickRow : disable?function(){}:ondg_wxDialogueLsClickRow
		}); */
	}
	
    function ondg_wxDialogueLsClickRow(index) {
    	$("#dg_wxDialogueLs").datagrid("acceptChanges");
		$("#dg_wxDialogueLs").datagrid("unselectAll");
    	$("#dg_wxDialogueLs").datagrid("selectRow", index).datagrid("beginEdit", index);
    }
	...
	function addDict() {
        ...
        $("#dg_wxDialogueLs").datagrid('insertRow',{index : 0,  row: data}); 
	}
    
    // 移除数据对话记录项
    function removeDict() {
        var rows = $("#dg_wxDialogueLs").datagrid("getSelections");
        var copyRows = [];
        for (var j= 0; j < rows.length; j++) {
			copyRows.push(rows[j]);
		}
        for(var i =0;i<copyRows.length;i++) {    
            var index = $("#dg_wxDialogueLs").datagrid("getRowIndex", copyRows[i]);
            $("#dg_wxDialogueLs").datagrid("deleteRow", index); 
		}
	}
	...
	function  formatterDate (date){
		...
		return dateObj.formatDate("yyyy-MM-dd hh:mm:ss");
	}
