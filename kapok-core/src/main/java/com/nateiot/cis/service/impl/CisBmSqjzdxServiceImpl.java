...
@Service("cisBmSqjzdxService")
@Transactional
public class CisBmSqjzdxServiceImpl extends BaseServiceImpl<CisBmSqjzdxDao, CisBmSqjzdx, Integer> implements CisBmSqjzdxService  {

	@Autowired
	private CisBmSqjzdxDao cisBmSqjzdxDao;
	
	@Autowired
	public CisBmSqjzdxServiceImpl(CisBmSqjzdxDao d) {
		super(d);
	}
	
	/*
	 * 查询 存在 社区矫正对象 人员信息(non-Javadoc)
	 * 
	 */
	@Override
	public Map<String,Object>  getSqjzdxList(Map<String, SearchFilter> conditions, Pageable pageable ){
		...
		try {
			...
			Page<Map<String, Object>> page = cisBmSqjzdxDao.getSqjzHouseHolder(conditions, pageable);
	...
	public Map<String,Object> softDel(int householderId){
		resetResultMap();
		try{
			CisBmSqjzdx bean = cisBmSqjzdxDao.findByHouseholderId(householderId);
			bean.setDelSign("Y");
			super.doSave(bean);
			resultMap.put(RESULT_CODE,0);
	...
	public Map<String, Object> doDelete(Integer householderId) {
		resetResultMap();
		CisBmSqjzdx bean =null ;
		try {
			bean =cisBmSqjzdxDao.findByHouseholderId(householderId);
			if(bean!=null){
				resultMap = super.doDelete(bean.getJzId());
			}else{
				resultMap.put(RESULT_CODE, -1);
	...
	public  CisBmSqjzdx getCisBmSqjzdxByHouseholderId(int householderId) {
		return cisBmSqjzdxDao.findByHouseholderId(householderId);
	}
