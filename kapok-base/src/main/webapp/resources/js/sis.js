...
(function($) {
	// 设置带有readonly属性的控件的样式
	var allInput = $("body").find(":input");
...
function setFormItemDisabled(form, disabled, exclude) {
	...
	allInput.each(function(index, element) {
		...
		if (exclude && exclude.indexOf(id) != -1) {
			return;
		}
		...
		if (clazz) {
			...
			if(clazz.indexOf("easyui-combogrid") != -1){
				$(element).combogrid(disabled?"disable":"enable");
			}
		} else {
			$(element).attr("disabled", disabled);
		}
...
// 设置行表Datagrid高度
function resetDatagridHeight(sub, subTabs) {
	var subHeight = $(sub).height();
	var tabsHeaderHeight = $(subTabs).find(".tabs-header").height();
	$(sub).find(".easyui-tabs").height(subHeight);
	$(subTabs).find(".tabs-panels").height(subHeight - tabsHeaderHeight);
	$(subTabs).find(".panel-body").height(subHeight - tabsHeaderHeight);
}
